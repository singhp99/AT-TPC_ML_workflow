...
def test_outlier_detection1():
    data = np.array([[[-200,14,500,1000],[5,6,7,8],[9,10,11,12],[2,2,2,2],[1,1,1,1]],[[10,20,30,40],[90,10,60,70],[50,60,70,80],[-150,25,350,450],[-80,15,250,300],[1,1,1,1],[2,2,2,2]],[[15,25,35,45],[95,15,65,75],[4,4,4,4],[3,3,3,3]]])
    #
...
def test_outlier_detection2():
    data = np.array([[[-1000,14,500,1000],[5,6,7,8],[9,10,11,12],[2,2,2,2],[1,1,1,1]],[[10,20,30,40],[90,10,60,70],[15,600,70,80],[-150,25,2000,450],[-80,15,250,300],[1,1,1,1],[2,2,2,2]],[[15,25,35,45],[95,15,65,75],[4,4,4,4],[3,3,3,3]]])
    
    event_length = np.array([5,7,4])
    outlier_detector = OutlierDetection(data)
    outlier_removed, new_event_len = outlier_detector.transform(data, event_length)
...
def test_up_down_scaling():
    data = np.array([[[1,2,3,4],[5,6,7,8]],[[10,20,30,40],[90,10,60,70],[50,60,70,80]],[[10,20,30,40],[90,10,60,70],[50,60,70,80],[10,20,30,40],[90,10,60,70],[50,60,70,80]]])
    event_length = np.array([2,3,6])
    target_size = 5
...
def test_scaling_data():
    ...
    assert np.all(scaled_data[:,:,0] <= 1) and np.all(scaled_data[:,:,0] >= -1) #x values should be scaled between -1 and 1
    assert np.all(scaled_data[:,:,1] <= 1) and np.all(scaled_data[:,:,1] >= -1) #y values should be scaled between -1 and 1
    assert np.all(scaled_data[:,:,2] <= 1) and np.all(scaled_data[:,:,2] >= -1) #z values should be scaled between -1 and 1